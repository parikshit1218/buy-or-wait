...
def simulate_unified(state, user_fact=None, proposed_payments=(), spending_changes=()):
    stop_events = set()
    reduce_events = {}
    ...
    # 3. Unified recurring income projection
    past_credits = [
        e for e in state.events
    ...
    if not is_final:
        # Group credits by stream
        inc_groups = defaultdict(list)
        for e in past_credits:
            if e.category == 'salary' or e.event_type.lower() == 'income':
                key = (e.category, e.description if 'freelance' in e.description.lower() or 'contract' in e.description.lower() else '')
                inc_groups[key].append(e)
                
        # If user has general salary
        if not inc_groups and past_credits:
            inc_groups[('salary', '')] = past_credits
        ...
        for key, evts in inc_groups.items():
            ordered = sorted(evts, key=lambda x: x.settlement_date or x.event_date)
            dates = [x.settlement_date or x.event_date for x in ordered]
            if len(dates) == 1:
                interval = 30
                ...
                intervals = [(r - l).days for l, r in zip(dates, dates[1:]) if (r - l).days > 0]
                interval = int(median(intervals)) if intervals else 30
            if interval <= 0:
                interval = 30

            last_evt = ordered[-1]
            last_date = dates[-1]
            salary_amt = last_evt.amount_home_currency
            ...
            while next_d <= end_date:
                pay_d = next_d
                if first_occurrence and delayed_date and delayed_date >= state.request_date:
                    pay_d = delayed_date
                first_occurrence = False
                
                if pay_d >= state.request_date:
                    if pay_d not in actual_future_inc_dates and not any(abs((a - pay_d).days) <= 7 for a in actual_future_inc_dates):
...
print("Testing samples with unified income projection:")
match_safe = 0
match_earliest = 0
for s in samples:
    req_id = s['request_id']
    ...
    # Safe now
    daily, lowest, lowest_d, margin = simulate_unified(state, fact)
    ...
    for day_idx in range(91):
        ...
        _, _, _, cand_margin = simulate_unified(state, fact, proposed_payments=[(cand_d, req_amt)])
